<template>
    <div class="page_body">
        ...
        <p v-if="!this.posts.length && !this.loading && !this.p_loading && !this.search_result_stat" style="color: black; font-size: 24px; font-weight: 900;text-align: center;width: 100vw;position: fixed; top: 50vh;left: 160px;">
        ...
        <div ref="main" style="z-index: 9;" class="main">
            ...
            <div v-if="!this.loading && !this.p_loading" class="posts_main" ref="posts_main" :class="{'no_overflow': !this.posts.length}" style="">
                <!-- <div v-for="(sp, i) in this.posts" :ref="'post_' + i" :key="i" class="card_cont" style="">
                    <img @click="$router.push('/post/' + sp.id)" class="card_img" :src="sp.image" alt="">
                ...
                <div class="card_cont" v-for="(sp, i) in this.posts" :ref="'post_' + i" :key="i">
                    <img @click="$router.push('/post/' + sp.id)" class="card_img" :src="sp.image" alt="">
                    <div class="card_text_cont">
                        <div style="width: 40px;" :class="{'bg_red': i%3 == 1, 'bg_yellow': i%3 == 2, 'bg_blue': i%3 == 0}" ></div>
                        <div style="display: flex; flex-direction: column;justify-content: center;min-height: 100px;">
                            <p style="font-weight: 900;font-size: 28px;color: black;padding: 0px;margin: 0px;text-align: left;margin-left: 40px;padding-top: 20px;">{{ sp.title }}</p>
                            <p style="font-weight: 600; font-size: 18px; color: #9E9E9E;padding: 0px;margin: 0px;text-align: left;padding-top: 5px;margin-left: 40px;padding-bottom: 20px;">{{ sp.description }}</p>
                        </div>
                        <img v-if="!sp.stars.includes(userid)" @click="sendStar(sp.id, i)" src="../assets/star.svg" style="width: 20px; height: 20px;align-self: flex-end;margin-bottom: 20px;margin-left: auto;margin-right: 30px;cursor: pointer;" alt="">
                        <img v-if="sp.stars.includes(userid)" @click="removeStar(sp.id, i)" src="../assets/star_alt.svg" style="width: 25px; height: 25px;align-self: flex-end;margin-bottom: 20px;margin-left: auto;margin-right: 30px;cursor: pointer;" alt="">
                        <img v-if="!sp.bucket.includes(userid)" @click="sendBucket(sp.id, i)" src="../assets/bucket.svg" style="width: 20px; height: 20px;align-self: flex-end;margin-bottom: 20px;margin-right: 30px;cursor: pointer;" alt="">
